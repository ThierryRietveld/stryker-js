...
import { Checker, CheckResult, CheckerTimeResult, MutantTime } from '@stryker-mutator/api/check';
...
export function create(injector: Injector<PluginContext>): CustomTypescriptChecker {
  return injector
    .provideFactory(commonTokens.logger, customTypescriptCheckerLoggerFactory, Scope.Transient)
    .provideClass('fs', HybridFileSystem)
    .injectClass(CustomTypescriptChecker);
}
...
export class CustomTypescriptChecker implements Checker {
  ...
  private readonly typescriptChecker: TypescriptChecker;
  private readonly mutantTimes: MutantTime[] = [];
